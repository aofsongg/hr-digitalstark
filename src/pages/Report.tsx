...
export default function Report() {
  ...
  useEffect(() => {
    ...
    if (term) filtered = filtered.filter(s => (s.EMP_NAME?.toLowerCase() || '').includes(term) || (s.EMP_LNAME?.toLowerCase() || '').includes(term) || (s.EMP_ID?.toLowerCase() || '').includes(term));
    if (selectedCompany !== 'all') filtered = filtered.filter(s => s.COMPANY_NM === selectedCompany);
