...
export default function EmployeeManagement() {
  ...
  useEffect(() => {
    ...
    setFilteredEmployees(employees.filter(e => e.COMPANY_NM.toLowerCase().includes(term) || e.EMP_NAME.toLowerCase().includes(term) || e.EMP_LNAME.toLowerCase().includes(term) || (e.NICK_NAME?.toLowerCase() || '').includes(term) || (e.NICK_NAME?.toLowerCase() || '').includes(term) || e.EMAIL.toLowerCase().includes(term)));
  }, [searchTerm, employees]);

  const handleOpenDialog = (emp?: Employee) => {
    if (emp) {
      setEditingEmployee(emp);
      setFormData({ COMPANY_NM: emp.COMPANY_NM, EMAIL: emp.EMAIL, EMP_ID: emp.EMP_ID,TITLE:emp.TITLE, EMP_NAME: emp.EMP_NAME, EMP_LNAME: emp.EMP_LNAME, NICK_NAME: emp.NICK_NAME || '', IDENTIFY_NUMBER: emp.IDENTIFY_NUMBER || '', POSITION_NM: emp.POSITION_NM || '', DEPARTMENT_NM: emp.DEPARTMENT_NM || '', START_WORKING_DATE: emp.START_WORKING_DATE ? new Date(emp.START_WORKING_DATE) : null, BASE_SALARY: emp.BASE_SALARY, BANK_NAME: emp.BANK_NAME || '', BANK_ACC_NUMBER: emp.BANK_ACC_NUMBER || '', BANK_ACC_NAME: emp.BANK_ACC_NAME || '' });
    } else {
      setEditingEmployee(null);
      setFormData({ COMPANY_NM: '', EMAIL: '', EMP_ID: '',TITLE:'', EMP_NAME: '', EMP_LNAME: '', NICK_NAME: '', IDENTIFY_NUMBER: '', POSITION_NM: '', DEPARTMENT_NM: '', START_WORKING_DATE: null, BASE_SALARY: 0, BANK_NAME: '', BANK_ACC_NUMBER: '', BANK_ACC_NAME: '' });
    }
    ...
    <DashboardLayout>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}><DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{editingEmployee ? 'Edit Employee' : 'Add Employeeใหม่'}</DialogTitle></DialogHeader>
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 gap-4"><div className="grid gap-2"><Label>Title</Label><Select value={formData.TITLE} onValueChange={v => setFormData({...formData, TITLE: v})}><SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger><SelectContent className="bg-popover">{TITLES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select></div>
           <div className="grid gap-2"><Label>First Name *</Label><Input value={formData.EMP_NAME} onChange={e => setFormData({...formData, EMP_NAME: e.target.value})} /></div><div className="grid gap-2"><Label>Last Name *</Label><Input value={formData.EMP_LNAME} onChange={e => setFormData({...formData, EMP_LNAME: e.target.value})} /></div><div className="grid gap-2"><Label>Nickname</Label><Input value={formData.NICK_NAME} onChange={e => setFormData({...formData, NICK_NAME: e.target.value})} /></div></div>
